...
class Conv_bn_block(torch.nn.Module):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._conv = torch.nn.utils.spectral_norm(torch.nn.Conv2d(*args, **kwargs))
...
class Res_block(torch.nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        self._conv1 = torch.nn.utils.spectral_norm(torch.nn.Conv2d(in_channels, in_channels//4, kernel_size = 1, stride =1))
        self._conv2 = torch.nn.utils.spectral_norm(torch.nn.Conv2d(in_channels//4, in_channels//4, kernel_size = 3, stride = 1, padding = 1))
        
        self._conv3 = torch.nn.utils.spectral_norm(torch.nn.Conv2d(in_channels//4, in_channels, kernel_size = 1, stride=1))
...
class encoder_net(torch.nn.Module):
    ...
    def __init__(self, in_channels, get_feature_map = False):
        ...
        self._pool1 = torch.nn.utils.spectral_norm(torch.nn.Conv2d(in_channels = self.cnum, out_channels = 2 * self.cnum, kernel_size = 3, stride = 2, padding = calc_padding(temp_shape[0], temp_shape[1], 3, 2)))
        ...
        self._pool2 = torch.nn.utils.spectral_norm(torch.nn.Conv2d(in_channels = 2*self.cnum, out_channels = 4* self.cnum, kernel_size = 3, stride = 2, padding = calc_padding(temp_shape[0], temp_shape[1], 3, 2)))
        ...
        self._pool3 = torch.nn.utils.spectral_norm(torch.nn.Conv2d(in_channels = 4*self.cnum, out_channels = 8* self.cnum, kernel_size = 3, stride = 2, padding = calc_padding(temp_shape[0], temp_shape[1], 3, 2)))
...
class decoder_net(torch.nn.Module):
    ...
    def __init__(self, in_channels, get_feature_map = False, mt =1, fn_mt=1):
        ...
        self._deconv1 = torch.nn.utils.spectral_norm(torch.nn.ConvTranspose2d(8*self.cnum, 4*self.cnum, kernel_size = 3, stride = 2, padding = calc_inv_padding(temp_shape[0], temp_shape[1], 3, 2)))
        ...
        self._deconv2 = torch.nn.utils.spectral_norm(torch.nn.ConvTranspose2d(4*self.cnum, 2*self.cnum, kernel_size =3 , stride = 2, padding = calc_inv_padding(temp_shape[0], temp_shape[1], 3, 2)))
        ...
        self._deconv3 = torch.nn.utils.spectral_norm(torch.nn.ConvTranspose2d(2*self.cnum, self.cnum, kernel_size =3 , stride = 2, padding = calc_inv_padding(temp_shape[0], temp_shape[1], 3, 2)))
...
class text_conversion_net(torch.nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        self._sk_out = torch.nn.utils.spectral_norm(torch.nn.Conv2d(self.cnum, 1, kernel_size =3, stride = 1, padding = 1))
        ...
        self._t_out = torch.nn.utils.spectral_norm(torch.nn.Conv2d(2*self.cnum, 3, kernel_size = 3, stride = 1, padding = 1))
    ...
    def forward(self, x_t, x_s):
        ...
        y_sk_out = torch.sigmoid(self._sk_out(y_sk))  # (0,1)
        
        y_t = self._t_decoder(x, fuse = None)
        
        # y_sk = torch.zeros_like(y_sk)
        y_t = torch.cat((y_sk, y_t), dim = 1)
        y_t = self._t_cbr(y_t)
        y_t_out = torch.tanh(self._t_out(y_t))  # (-1,1)
...
class inpainting_net(torch.nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        self._out = torch.nn.utils.spectral_norm(torch.nn.Conv2d(self.cnum, 3, kernel_size = 3, stride = 1, padding = 1))
...
class Discriminator(torch.nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        self.cnum = 32
        self._conv1 = torch.nn.utils.spectral_norm(torch.nn.Conv2d(in_channels, 64, kernel_size = 3, stride = 2, padding = calc_padding(temp_shape[0], temp_shape[1], 3, 2)))
        self._conv2 = torch.nn.utils.spectral_norm(torch.nn.Conv2d(64, 128, kernel_size = 3, stride = 2, padding = calc_padding(temp_shape[0], temp_shape[1], 3, 2)))
    
        # self._conv2_bn = torch.nn.BatchNorm2d(128)
        
        self._conv3 = torch.nn.utils.spectral_norm(torch.nn.Conv2d(128, 256, kernel_size = 3, stride = 2, padding = calc_padding(temp_shape[0], temp_shape[1], 3, 2)))
        
        # self._conv3_bn = torch.nn.BatchNorm2d(256)
        
        self._conv4 = torch.nn.utils.spectral_norm(torch.nn.Conv2d(256, 512, kernel_size = 3, stride = 2, padding = calc_padding(temp_shape[0], temp_shape[1], 3, 2)))
        
        # self._conv4_bn = torch.nn.BatchNorm2d(512)
        
        self._conv5 = torch.nn.utils.spectral_norm(torch.nn.Conv2d(512, 1,  kernel_size = 3, stride = 1, padding = 1))
